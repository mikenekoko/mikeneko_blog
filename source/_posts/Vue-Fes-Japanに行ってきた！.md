...
* そう、ブラウザーがなくても開発できる環境になるのです。
* 状態を変更した場合、それは複数のコンポーネントに影響を与えます。
* この場合にどのコンポーネントにそれを適応させたらいいのか考えてくれます。
** ↑フレームワークなので、そこをすべて理解しておく必要はありません。

## よりネイティブ向けに作りやすく。
* カスタムレンダラAPI
Vue3では、カスタムレンダラAPIというものを提供します。これを使うことで、Neitive向けのアプリを使いやすくなります。
=> Vueネイティブはあまり聞かなかったが、これで李アクトに勝てるようになる！？
* これは、ネイティブ向けの開発者ツールになります。

### デモ
1. Node operationというものをAPIに渡す
2. 関数を呼ぶことで、カスタムレンダラAPIをy日出します。


実際にDOｍに描画しなくても、Javascriptとして受け取ることで、Canvasといったもので描画が可能となる。
レンダラー memo

## よりあなたのコードの保守性を向上
リアクティビティAPI
* このAPIを使うことで、どんなコンポーネントでも監視できるようになります。Vuexもです。
Storeを変更することですべての参照している箇所をアップデートできることができます。
今後、Vuexは必要ないかもしれませんね。 => マジで！？

デモで実際にRootだけでなくすべてが変わっているのを見せてくれた。

### コンポーネントの詐病がを理解する
＊ 新しく、RenderTrackという仕組みを導入します。

## TSXによるTypeScriptサポートの強化
* Vue2のTSのサポートはそんなにいいものではありませんでした。
* クラスのコンポーネントAPIをネイティブに扱える。APIのランタイムからコンポーネントをIMぽｒｔして、Extendsすることもできる。
* TSXでRender関数を記述することができる。
* Propsの肩も表示される。

## 実践的なHooksAPIの話
* これはReactで登場した機能です。
* コンポーネントのロジックを梱包するのが簡単になります。
* 複数のコンポーネントの扱いのため、Mix-inという仕組みがあります。
...
たとえば1つのコンポーネントがやたらレンダリングに時間がかかるとします。
個のコンポーネントが1秒かかり、それが200個あった場合200秒になりますよね。
* 入力フォームに入れた内容を、コンポーネントで読み込むとします。
* それが200個あれば、200個のコンポーネントが更新されますよね。
* 入力されるたびにコンポーネントが更新され、おもくなります
* 更新をブラウザが描画するまで固まりますね。

TimeSlicingを含んだVueに切り替えるだけで、完全にレスポンシブになります。
* で、その仕組みはというと、開発者ツールで見てみると、
* 高速でタイピングしたとして、hoge と売った場合、hog は描画するものの、最後のeが描画された瞬間に最初の3文字は必要なくなります。これをslicingは自動で判断し、処理をスキップします。
...
# 第一セッション Bホール : Vue.js と Web Components のこれから
* 80点
  * すぐに弊社で使えそう
